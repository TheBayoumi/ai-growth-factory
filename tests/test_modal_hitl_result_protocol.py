...
class ModalHitlResultProtocolTests(unittest.TestCase):
    ...
    def test_preproduction_has_explicit_local_json_transport_and_volume_fallback(self) -> None:
        source = (self._repo_root() / "cloud" / "modal_vimax_hitl_preproduction.py").read_text(encoding="utf-8")
        self.assertIn('@app.local_entrypoint()', source)
        self.assertIn('HITL_RESULT_JSON=', source)
        self.assertIn('prepare_vimax_keyframe_review.remote(clean_sha)', source)
    ...
    def test_manual_hitl_workflow_exports_before_classifying_human_gate(self) -> None:
        ...
        self.assertIn('modal run -m cloud.modal_vimax_hitl_preproduction --code-sha "$CODE_SHA"', source)
        self.assertIn('HITL_RESULT_JSON=', source)
