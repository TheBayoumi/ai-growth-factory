...
def install_production_runtime() -> None:
    ...
    Publisher names are canonicalized from selected evidence URLs before package
    validation. Scene attribution is then handled by factory.source_attributed_llm at
    the exact package boundary. Near-complete narration length is stabilized before the
    editorial wrapper. Unsupported publisher relationships are grounded in explicit
    independent-source language before the same editorial validator runs. Incomplete but
    otherwise valid Qwen Omni feedback is normalized before the bounded selective voice
    repair loop executes. Executable visual prompts must stay semantically distinct, Wan
    motion is bound to each scene's actual subject, and temporal QC excludes the separate
    caption layer while retaining strict checks for generated video scenes.
    """
    global _INSTALLED
    if _INSTALLED:
        return
